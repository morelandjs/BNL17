...
\begin{frame}[t]{Bayesian methodology for heavy-ion collisions}
  ...
  \begin{tcolorbox}[colback=theme!10, colframe=theme!0]
    \begin{itemize}
      \item[\color{offblack} \scriptsize \faicon{exclamation-triangle}] Hydro for heavy-ion collisions not trusted on same level as e.g.\ GR for gravitational waves\\[1ex]
...
\begin{frame}{Leveraging data from RHIC beam energy scan}
  \medskip
  \includegraphics<1>[height=0.7\textheight]{corner_19}
  \includegraphics<2>[height=0.7\textheight]{corner_39}
  \includegraphics<3-4>[height=0.7\textheight]{corner_62}
  \begin{tikzpicture}[remember picture, overlay]
    \only<1>{\node[align=left] at (-1.8, 5.8) {\Large $\sqrt{s_{NN}}=19$~GeV};}
    \only<2>{\node[align=left] at (-1.8, 5.8) {\Large $\sqrt{s_{NN}}=39$~GeV};}
    \only<3>{\node[align=left] at (-1.8, 5.8) {\Large $\sqrt{s_{NN}}=62$~GeV};}
    \only<1-3>{\node[align=left, text width=5.5cm] at (2, 4){\large Left: Bayesian posterior for hydrodynamic model parameters calibrated at different beam energies $\sqrt{s_{NN}}$};}
    \only<4>{\node[align=center] at (2, 3.8) {
      \smallskip 
      \small \quad Beam-energy dependence of $\langle \eta/s \rangle$\\
      \includegraphics[scale=.28]{bes_CL90_etas}
    };}
  \end{tikzpicture}
  \begin{columns}[T]
    \begin{column}{0.025\textwidth}
